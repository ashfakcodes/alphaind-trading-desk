...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Dual_LLM-Qwen_3.8_Max_%2B_Claude_3.5-8A2BE2?style=for-the-badge" alt="Dual LLM" />
...
| :--- | :--- | :--- |
| **Natural-Language LUI Command Bar** | Real-time semantic parsing and entity extraction via dual LLM engine (`Qwen 3.8 Max` & `Claude 3.5 Sonnet`) | Formless, instantaneous trade auditing from plain English prompts. |
...
### Option A: One-Click Local Monolithic Runner
Runs both the FastAPI backend and single-screen Obsidian HUD on `http://localhost:8000`:
```bash
# 1. Clone repository
...
# 3. Launch Monolithic Desk Runner
python run.py
```
...
### Option B: Decoupled Frontend & Backend (Production Development)
Run backend and frontend as distinct microservices:
...
Spin up the complete containerized stack in isolated environments:
```bash
...
### Zero-Math-Hallucination Guarantee
...
* **LLM Strengths Leveraged:** **Bitget Qwen 3.8 Max** and **OpenRouter Claude 3.5 Sonnet** handle natural language intent deconstruction, semantic entity extraction, and conversational synthesis of complex quantitative metrics into clear, actionable prose.
...
# Primary LLM Engine (Bitget Qwen 3.8 Max)
...
# Fallback LLM Engine (OpenRouter Claude 3.5 Sonnet)
